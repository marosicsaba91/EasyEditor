...
namespace EasyEditor
{
	...
	public static partial class EditorHelper
	{
		...
		{
			Dictionary<IconSize, Texture> variants = new();

			foreach (string searchName in strings)
			{
				GUIContent guiContent = EditorGUIUtility.IconContent(searchName);
				Texture icon = guiContent.image;

				if (icon == null)
					continue;

				Vector2Int size = new(icon.width, icon.height);
				float max = Mathf.Max(size.x, size.y);
				IconSize? iconSize = max > 16 ? IconSize.Big : IconSize.Small;

				if (!variants.TryAdd(iconSize.Value, icon))
					Debug.LogWarning($"Failed to add {iconType} {iconSize} {icon}");
			}
		...
		public static Texture GetIcon(IconType iconType, IconSize size = IconSize.Small)
		{
			...
			return variants.FirstOrDefault().Value;
		}

		public static readonly Dictionary<IconType, string[]> searchNames = new()
		{
